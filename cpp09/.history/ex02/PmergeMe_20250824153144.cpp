...
for (std::deque<int>::iterator it = pend.begin(); it != pend.end(); ++it)
{
    ...
    int index = insertion_it->at(); // Utiliza at() en lugar de operator[]
    std::cout << "Insertando en posición " << index << std::endl;
    if (index >= s.size()) {
        std::cerr << "Error: índice fuera de límites" << std::endl;
        exit(1);
    }
